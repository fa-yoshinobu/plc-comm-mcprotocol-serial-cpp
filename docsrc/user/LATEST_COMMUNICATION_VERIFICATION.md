...
| MELSEC Q-series `Q06UDVCPU` | `QJ71C24N` | `melsec:q` | RS-232C, MC Protocol serial | CPU model, contiguous read/write, supported-device soak, random, multi-block, monitor, host/module buffer, link-direct, and native-qualified `Un\G`. C4 Binary / Format5 and C4 ASCII / Format4 are confirmed for the validated routes. | Some timer/counter random-read items have route-specific limitations; helper `Un\G` is not used as a fallback. The serial module and client must use the same MC protocol format. | QJ71C24N RS-232C report |
