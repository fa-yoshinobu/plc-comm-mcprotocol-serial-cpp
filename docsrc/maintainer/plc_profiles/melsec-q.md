# `melsec:q` Profile Specification

Status: target-validated maintainer specification for the tested Q-series setup.

This file records the `melsec:q` specification confirmed on the local MELSEC-Q
...
These are confirmed for `melsec:q` on the tested Q target.
...
buffer value as the native route; reject that helper route for `melsec:q`
...
therefore excluded from `melsec:q` support.
