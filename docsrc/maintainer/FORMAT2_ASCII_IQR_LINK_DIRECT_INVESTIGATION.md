# 4C ASCII Format2 iQ-R Link-Direct Investigation
...
| Profile / frame | `melsec:iq-r`, `c4-ascii-f2` |
| Symptom | Link-direct responses could terminate the CLI with access violation `0xC0000005`. |
| Root cause | `FrameCodec::decode_response` searched for `ETX` before checking that the received chunk was at least the Format2 STX response prefix length. A short serial chunk could make the search start past the span end. |
...
After rebuilding the fixed binary, Format2 passed the previously failing access
routes:
...
- `Un\G` / `Un\HG` native-qualified reads
- Full Format2 suite: link-direct read/write, random, multi-block, and monitor routes

SLMP monitor cross-check also confirmed values written through Format2 MC
Serial were visible through SLMP monitor read.

## Non-Bug Observations

These observations are not Format2 defects:
...
No active Format2 follow-up remains.
