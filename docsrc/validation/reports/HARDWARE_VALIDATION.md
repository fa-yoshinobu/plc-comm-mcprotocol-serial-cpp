...
| Clear error information | `1617` via `error-clear` | native pass | validated on both `R120PCPU / RJ71C24-R2 / --series iqr` and the `Format1 ASCII / --series ql` setup |
| Remote RUN/STOP/PAUSE | `1001/1002/1003` via `remote-run` / `remote-stop` / `remote-pause` | native pass | validated on both `R120PCPU / RJ71C24-R2 / --series iqr` and the `Format1 ASCII / --series ql` setup; the final `remote-run` restored the original state |
