...
| `Jn\...` native probes | batch read/write plus native `0403`, native `1402`, and native `0801/0802` are hardware-validated on `J1\W10`; native multi-block still lacks dedicated hardware coverage |
...
| User frame / serial-module control extras | pass | `0610/1610` write-read-delete, `1615`, `1618`, and `0631` passed on the focused recheck; `0610` read-back returned `frame-bytes=0` on flash user-frame `0x03E8` even though registration data matched, while buffer-memory frame `0x8001` returned `frame-bytes=5` |
