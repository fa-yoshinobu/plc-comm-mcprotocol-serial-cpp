...
| Remote password unlock/lock | target-dependent / unresolved | focused `--series iqr` checks with a `10`-character test password returned `0x7F22` for both `lock` and `unlock`; read-only `cpu-model` and `read-words D0 1` remained available |
