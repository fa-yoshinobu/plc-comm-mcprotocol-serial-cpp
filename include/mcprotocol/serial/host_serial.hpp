...
/// \brief Host-side serial-port configuration used by `HostSerialPort`.
///
...
[[nodiscard]] inline Status validate_serial_config(const HostSerialConfig& config) noexcept {
...
/// \brief Minimal blocking host-side serial-port wrapper used by host tools.
///
/// This class is not required on MCU targets. It exists so the same request/response codec and
/// client logic can be exercised from host-side validation tools on Windows and POSIX systems.
class HostSerialPort {
 public:
  ...
  [[nodiscard]] Status open(const HostSerialConfig& config) noexcept;
...
/// \brief One-release compatibility alias for `HostSerialConfig`.
using PosixSerialConfig [[deprecated("use HostSerialConfig")]] = HostSerialConfig;
/// \brief One-release compatibility alias for `HostSerialPort`.
using PosixSerialPort [[deprecated("use HostSerialPort")]] = HostSerialPort;
